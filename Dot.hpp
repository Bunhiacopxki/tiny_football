...
public:
    ...
    Dot(bool isMainDot = false, int team = 0, int playerid = 0);
...
Dot::Dot(bool isMainDot, int team, int playerid)
{
    ...
    if (!isMainDot)
    {
        if (team == 1)
        {
            mPosX = rand() % (SCREEN_WIDTH / 2);  // Nửa trái màn hình
            mPosY = rand() % (SCREEN_HEIGHT / 2); // Nửa trên màn hình
        }
        else if (team == 2)
        {
            mPosX = (SCREEN_WIDTH / 2) + rand() % (SCREEN_WIDTH / 2);   // Nửa phải màn hình
            mPosY = (SCREEN_HEIGHT / 2) + rand() % (SCREEN_HEIGHT / 2); // Nửa dưới màn hình
        }
    }
    else
    {
        if (playerid == 1)
        {
            mPosX = 50; // Dot chính 1
            mPosY = SCREEN_HEIGHT / 2;
        }
        else if (playerid == 2)
        {
            mPosX = SCREEN_WIDTH - DOT_WIDTH - 50; // Dot chính 2
            mPosY = SCREEN_HEIGHT / 2;
        }
    }
    if (team == 1)
...
void Dot::handleEvent(SDL_Event &e, Ball &ball, std::vector<Dot> &players, KickMeter kickMeter)
{
    ...
    if (e.type == SDL_KEYDOWN && e.key.repeat == 0)
    {
        ...
        if (playerID == 1)
        {
            ...
            // {
            //     // if (ball.isPassing || !(ball.owner == this)) // Chỉ cầu thủ có playerID == 1 mới được chuyền bóng
            //     // 	break;
            // }
            // printf("Mvelx2: %lf; Mvely2: %lf\n", mVelX, mVelY);
        }
